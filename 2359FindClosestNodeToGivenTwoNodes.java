class Solution {
    ...
    public int closestMeetingNode(int[] edges, int node1, int node2) {
        ...
        int minDistNode = -1, minDistTilNow = Integer.MAX_VALUE;
        for (int currNode = 0; currNode < n; currNode++) {
            if (minDistTilNow > Math.max(dist1[currNode], dist2[currNode])) {
                minDistNode = currNode;
                minDistTilNow = Math.max(dist1[currNode], dist2[currNode]);
            }
        }
        return minDistNode;
    }
